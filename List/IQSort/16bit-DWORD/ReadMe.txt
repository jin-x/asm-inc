...
Иногда при сортировке массивов необходимо упорядочить не только числовые элементы, но и соответствующие им данные. Например, имеется список людей с указанием даты рождения и фамилии для каждого из них. Дата рождения записана в виде числового значения типа DWORD (младший байт – число, второй байт – месяц, старшее слово – год), а фамилия – в виде дальнего указателя на строку. Таким образом, каждый элемент массива будет содержать 2 записи размером DWORD каждая:
  +0  DWORD  Дата рождения
  +4  DWORD  Адрес строки с фамилией
При сортировке такого массива по дате рождения необходимо сравнивать только первые элементы (опорные значения), однако перемещать (менять местами) и первые, и вторые.
Такой массив может содержать во втором двойном слове 2 элемента с ближними указателями (например, фамилию и имя). Если же массив должен содержать более 3-х элементов, все элементы, кроме опорного (например, фамилию, имя, адрес и телефон) можно записать в отдельную область памяти, а во втором поле указать ссылку на эту структуру.
...
Данная коллекция модулей работает только при включенной поддержки инструкций процессора 386+
Редакцию модулей, позволяющих работать с 16-битными данными (элементами типа WORD) можно также скачать по ссылке, указанной в заголовках файлов или в конце данного текста.
...
Процедуры модифицируют регистры общего назначения, кроме регистра BP (и SP, разумеется), причём старшее слово EBP также изменяется. Некоторые из процедур (InsSort4, InsSort4DE) также сохраняют CX и DX, но лучше на это не опираться, поскольку в будущих версиях (если они будут) это не гарантируется.
...
Файлы с именами без суффиксов (S1, S2) содержат много директив условной компиляции и констант и могут быть сложны для понимания. Однако они более универсальны и включают в код только ту часть, которая необходима. Например, вы можете указать необходимые настройки в исходном коде своей программы, *после* чего включить процедуру сортировки следующим образом:
  ...
  inclInsSort4 = 1	; включить в код полную процедуру сортировки вставками для использования её отдельно от процедуры умной быстрой сортировки (по умолчанию включается только используемая процедурой IQSort часть)
  Sort4SignCmp = 0	; беззнаковое сравнение
  Sort4Ascending = 0	; сортировка значений по убыванию
  Sort4ElemDWords = 2	; элементы массива имеют 2 двойных слова в каждом элементе (допускается использовать только значение 1 или 2)
  IQSort4InsThrs = 16	; использовать сортировку вставками для массивов, в которых меньше 16 элементов [это значение установлено по умолчанию]
  IQSort4MaxStk = 128	; максимальный размер стека в байтах, который допустимо использовать процедуре IQSort (включая call из основного кода) [это значение установлено по умолчанию]
  INCLUDE IQSort4.inc	; включить процедуру в проект

Единственной требуемой настройкой является SortElemDWords, остальные могут быть опущены – в этом случае используются значения по умолчанию. Подробное описание каждой настройки смотрите в исходниках :)
Далее для вызова процедуры сортировки необходимо загрузить требуемые значения в регистры и вызвать процедуру:
  ...
  call IQSort4		; вызов процедуры умной сортировки

Файлы с суффиксами S1 и S2 (Simple Single/Double) содержат минимальное кол-во настроек, которые хранятся внутри самих модулей (либо эти настройки отсутствуют вовсе). Для изменения некоторых настроек (например, присутствия знака в числовых значениях и направление сортировки) придётся править исходники (правда, сделать это легко – в этих файлах есть описание этого незамысловатого процесса). Эти файлы практически не содержат директив условной компиляции и более понятны для изучения.
...
examples\... – примеры использования процедур сортировки (тестовые программы)
  ...
  IQSort41.asm – умная быстрая сортировка массива с одиночными элементами по возрастанию, используя IQSort4.inc
  IQSrt1D.asm – умная быстрая сортировка массива с одиночными элементами по убыванию, используя IQSort4.inc
  IQSort42.asm – умная быстрая сортировка массива с двойными элементами по возрастанию, используя IQSort4.inc
  IQSrt42D.asm – умная быстрая сортировка массива с двойными элементами по убыванию, используя IQSort4.inc
  IQSrt4S1.asm – умная быстрая сортировка массива с одиночными элементами по возрастанию, используя IQSrt4S1.inc
  IQSrt4S2.asm – умная быстрая сортировка массива с двойными элементами по возрастанию, используя IQSrt4S2.inc

  QSort41.asm – обычная быстрая сортировка массива с одиночными элементами по возрастанию, используя QSort4.inc
  QSort41D.asm – обычная быстрая сортировка массива с одиночными элементами по убыванию, используя QSort4.inc
  QSort42.asm – обычная быстрая сортировка массива с двойными элементами по возрастанию, используя QSort4.inc
  QSort42D.asm – обычная быстрая сортировка массива с двойными элементами по убыванию, используя QSort4.inc
  QSort4S1.asm – обычная быстрая сортировка массива с одиночными элементами по возрастанию, используя QSort4S1.inc
  QSort4S2.asm – обычная быстрая сортировка массива с двойными элементами по возрастанию, используя QSort4S2.inc

  ISort41.asm – сортировка вставками массива с одиночными элементами по возрастанию, используя ISort4.inc
  ISort41D.asm – сортировка вставками массива с одиночными элементами по убыванию, используя ISort4.inc
  ISort42.asm – сортировка вставками массива с двойными элементами по возрастанию, используя ISort4.inc
  ISort42D.asm – сортировка вставками массива с двойными элементами по убыванию, используя ISort4.inc
  ISort4S1.asm – сортировка вставками массива с одиночными элементами по возрастанию, используя ISort4S1.inc
  ISort4S2.asm – сортировка вставками массива с двойными элементами по возрастанию, используя ISort4S2.inc
